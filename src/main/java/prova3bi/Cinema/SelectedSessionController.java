...
public class SelectedSessionController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		int emptyChair = 0;
		for (var chair : listSeat) {
			if (!chair.ocupada)
				emptyChair++;
		}

		txtTicketValue.setText(sessionSelected.preco + " R$");
		txtTitle.setText(sessionSelected.filme.title);
		
		txtSynopsis.setText(sessionSelected.filme.synopsis);
		txtSynopsis.setWrapText(true);
		
		txtQuantity.setText("Cadeiras disponiveis na sessão " + emptyChair);
